...
from fileio import join, make_clean_folder, get_rgbd_file_lists, check_folder_structure
...
def integrate_rgb_frames_for_fragment(color_files, depth_files, fragment_id, n_fragments, pose_graph_name, intrinsic, config):
    # return gt_trajectory as the ground truth, but use the pose graph for tsdf
    gt_trajectory = read_trajectory(config['path_dataset'].replace('-simulated/', '-traj.txt'))
    ...
    return mesh, gt_trajectory[fragment_id * config['n_frames_per_fragment']].pose


def make_pointcloud_for_fragment(path_dataset, color_files, depth_files, fragment_id, n_fragments, intrinsic, config):
    mesh, pose = integrate_rgb_frames_for_fragment(color_files, depth_files, fragment_id, n_fragments, join(path_dataset, config["template_fragment_posegraph_optimized"] % fragment_id), intrinsic, config)
    ...
    np.save(pcd_name.replace('ply', 'npy'), pose)
